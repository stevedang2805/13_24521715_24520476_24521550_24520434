"""
mogi_scraper.py  v3_modified
=======================
Thu thập tin rao từ mogi.vn và xuất ra DataFrame có cùng 104 cột
với dataset Chợ Tốt gốc (labeled_dataset_v6_NEWPRICE_RAW.csv).

Fixes v3_modified
-------------------
1. province_norm: "tphcm" → "tp ho chi minh" (khớp bảng giá mogi)
2. district_norm: "quan 2 (tp. thu duc)" → "quan 2"
3. house_type_norm: đọc từ URL slug + keyword fallback
4. usable_area_num: fallback = land_area_num khi không có riêng
5. Lọc bỏ tin ngoài HCM+HN
6. [FIX BUG] description_signature dùng full description, không truncate [:500]
7. Auto-retry khi mất mạng: thử lại tối đa MAX_RETRIES lần,
         mỗi lần chờ lũy tiến (2s → 4s → 8s → 16s → 32s)
         + checkpoint tự động lưu tiến độ vào file .checkpoint.json
         để resume nếu tắt máy giữa chừng
8. [FIX] Pagination Mogi dùng ?cp=2, ?cp=3 thay vì ?page=2, ?page=3
9. [FIX] URL mặc định dùng /mua-nha-dat cho HCM/HN
10. [NEW] Ghi CSV tạm sau mỗi tin hợp lệ để mở file thấy dữ liệu ngay

Cách dùng
---------
# Scrape live — HCM + HN mặc định:
py mogi_scraper_v3_modified.py --mode live --pages 130 --output mogi_hcm_hn.csv

# Scrape URL tùy chỉnh (per-type, per-district):
py mogi_scraper_v3_modified.py --mode live --listing-url "https://mogi.vn/tp-ho-chi-minh/mua-nha-hem-ngo" --pages 80 --output mogi_hem_hcm.csv

# Resume nếu bị ngắt giữa chừng (tự động đọc checkpoint):
py mogi_scraper_v3_modified.py --mode live --pages 130 --output mogi_hcm_hn.csv --resume

# Parse offline:
py mogi_scraper_v3_modified.py --mode offline --listing-html listing.html --detail-html detail.html --output out.csv

Yêu cầu
--------
pip install requests beautifulsoup4 pandas unidecode tqdm
"""
